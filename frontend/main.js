...
`;
  ...
  const contactsType = ["Телефон", "Доп.телефон", "Email", "Vk", "Facebook"];
  //очистка таблицы
  function cleanTable() {
    if (!clientsTable) {
      return;
  ...
  function renderClient(objClient) {
    ...
    $createdAt.textContent = toLocalFormat.format(
      new Date(objClient.createdAt)
    );
    $updatedAt.textContent = toLocalFormat.format(
      new Date(objClient.updatedAt)
    );
    ...
    $changeBtn.addEventListener("click", async (e) => {
      const response = await fetch(
        `http://localhost:3000/api/clients/${objClient.id}`
      );
  ...
  function createModalAddClient() {
    ...
    $pSurname.classList.add(
      "modal-form__placeholder",
      "modal-form__placeholder--invalid"
    );
    $pName.classList.add(
      "modal-form__placeholder",
      "modal-form__placeholder--invalid"
    );
    $pLastName.classList.add(
      "modal-form__placeholder",
      "modal-form__placeholder--invalid"
    );
  ...
  function placeInputPlaceholder(input, placeholder) {
    ...
    !input.value
      ? placeholder.classList.add(liftPlaceholder)
      : placeholder.classList.remove(liftPlaceholder);
  }
  ...
  function createContactSelect(box, contactsArray = false) {
    ...
    contactsArray.forEach((contact) => {
      ...
      contactsType.forEach((type) => {
        ...
        console.log(
          "contactsType.forEach contact.type === $option.value",
  ...
  Promise.all([getClientsArray()]).then((responses) => {
